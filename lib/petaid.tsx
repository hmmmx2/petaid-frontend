...
const mapPet = (p: ApiPet): Pet => ({
  ...
  // effective avatar emoji = chosen icon, else pet-type default
  emoji: p.icon_emoji || p.pet_type?.icon_emoji || "🐾", bg: p.pet_type?.icon_bg || "#F4F4F4",
  typeId: p.pet_type?.id, typeName: p.pet_type?.name || "", notes: p.health_notes || "",
  image: p.image_url || null,
  icon: p.icon_emoji || null,
});
...
export const petaid = {
  ...
  /* domain actions */
  addPet: (b: { name: string; pet_type_id: string; breed?: string; age_years?: number | null; health_notes?: string; image_url?: string | null; icon_emoji?: string | null }) =>
    req("/api/v1/pets", { method: "POST", body: JSON.stringify(b) }),
  updatePet: (id: string, b: { name?: string; pet_type_id?: string; breed?: string; age_years?: number | null; health_notes?: string; image_url?: string | null; icon_emoji?: string | null }) =>
